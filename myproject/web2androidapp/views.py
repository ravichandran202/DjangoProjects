...
def account_created_html_email(user):
    ...
    html_message = render_to_string('web2app/account-created-email.html',context=context)
...
def send_liked_html_email(user,liked_app_obj):
    ...
    html_message = render_to_string('web2app/liked-email.html',context=context)
    message = strip_tags(html_message)
    email_from = settings.EMAIL_HOST_USER
    recipient_list = [user.email,]
    try:
...
def send_commented_html_email(user,commented_app_obj):
    ...
    html_message = render_to_string('web2app/comment-email.html',context=context)
    message = strip_tags(html_message)
    email_from = settings.EMAIL_HOST_USER
    recipient_list = [user.email,]
    try:
...
def home(request):
    return render(request, 'web2app/index.html')
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect("web2app:home")
        else:
            messages.error(request, 'Username or Password is incorrect')
            return redirect("login")
    return render(request, 'web2app/login.html')
    ...
    if request.method == 'POST':
        ...
        if password1 == password2:
            if User.objects.filter(email=email).exists():
                ...
                return redirect('web2app:signup')
                ...
                return redirect('web2app:login')
        else:
            messages.error(request, "Please Enter same Password")
            return redirect('web2app:signup')
    return render(request, 'web2app/signup.html')
...
def profile(request,id):
    ...
    return render(request, 'web2app/profile.html', {
...
def editProfile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('web2app:profile',request.user.id)
    return render(request, 'web2app/editprofile.html',{
        'form' : form,
        'user_info': user,
    })
...
def upload(request):
    if request.method == 'POST':
        ...
        if AppDetails.objects.filter(app_name=app_title).exists():
            ...
            return redirect('web2app:upload')
        ...
        return redirect('web2app:download', created_app.id)
    return render(request, 'web2app/upload.html')
...
def app_store(request):
    ...
    return render(request, 'web2app/appstore.html',{
...
def app_page(request, id):
    ...
    return render(request, 'web2app/app-page.html',{
        'app' : AppDetails.objects.get(id=id),
...
def edit_app(request, id):
    ...
    if request.method == "POST":
        ...
        app.about = about_app
        app.save()
        return redirect('web2app:apppage', app.id)
    return render(request, 'web2app/editapp.html',{
        'app' : AppDetails.objects.get(id=id),
    })
...
def download(request, id):
    ...
    app.save()
    return render(request, 'web2app/download.html', {
        'app': app,
    })
...
def delete_comment(request,appid,id):
    ...
    comment.delete()
    return redirect('web2app:apppage', appid)
...
def like_app(request,id):
    ...
    return redirect('web2app:apppage', id)
